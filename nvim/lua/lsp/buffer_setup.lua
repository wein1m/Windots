...
local function setup_lsp_keymaps(_, bufnr)
  ...
  vim.keymap.set("n", "<leader>uS", vim.lsp.buf.references, map_opts);
  vim.keymap.set({ "n", "v" }, "Ji", vim.lsp.buf.code_action, map_opts);
end

-- setup diagnostic keymaps
local function setup_diagnostics_keymaps(_, bufnr)
  local map_opts = { silent = true, buffer = bufnr }
  vim.keymap.set('n', '[e', core.partial(vim.diagnostic.goto_prev, { float = true }), map_opts)
  vim.keymap.set('n', ']e', core.partial(vim.diagnostic.goto_next, { float = true }), map_opts)
  vim.keymap.set('n', ',', core.partial(vim.diagnostic.open_float, { focus = false }), map_opts)
  vim.keymap.set('n', '<leader>e', core.partial(lsp_utils.toggle_diagnostics_list, false), map_opts)

  -- toggle virtual_lines (diagnostic lines) or virtual_text (diagnostic blocks)
  vim.keymap.set('n', 'gK', core.partial(function()
    local toggle = not vim.diagnostic.config().virtual_lines
    vim.diagnostic.config({ virtual_lines = toggle })
  end, false), map_opts)
  vim.keymap.set('n', 'gJ', core.partial(function()
...
local function setup_formatting_keymaps(client, bufnr)
  ...
  if client.server_capabilities.documentFormattingProvider then
    vim.keymap.set('n', '<c-f>', core.partial(vim.lsp.buf.format, { async = true }), map_opts)
  end

  if client.server_capabilities.documentRangeFormattingProvider then
    vim.keymap.set('v', '<c-f>', core.partial(vim.lsp.buf.format, { async = true }), map_opts)
  end
end
